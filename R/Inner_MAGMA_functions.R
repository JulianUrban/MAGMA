...
#'
#' matches Cases iterativel during MAGMA
#'
#' This function conducts the matching process, by ectracting the match with
#' the lowest distance
#'
...
#' Inner MAGMA distance estimation preperation function
#'
...
build_value_matrix <- function(input_list, rep_element, name_ps = "distance_ps") {
  ...
  if(length(rep_element) == 2) {
    ...
    value_matrix[, 2] <- purrr::map(input_list[[2]][[name_ps]], rep, rep_element[1]) %>%
      unlist() %>%
      rep(., rep_element[3] * rep_element[4])
    value_matrix[, 3] <- purrr::map(input_list[[3]][[name_ps]], rep, rep_element[1] * rep_element[2]) %>%
      unlist() %>%
      rep(., rep_element[4])
